...
Before(async function () {
  await actor.navigateTo(pages.home + pages.todoList);
});
...
When(
  ...
  async ( todoItemNew ) => {
    await actor.updateTodoItem(todoItemsFilter[1], todoItemNew);
  }
);
...
When(
  ...
  async ( todoItemNew ) => {
    await actor.updateTodoItem(todoItemsFilter[0], todoItemNew);
  }
);
...
When("User deletes an active todo item", async () => {
  await asserter.verifyActiveTodoCount(2);
  await asserter.verifyTotalTodoCount(3);
  await actor.deleteTodoItem(todoItemsFilter[1]);
});

When("User deletes a completed todo item", async () => {
  await asserter.verifyActiveTodoCount(2);
  await asserter.verifyTotalTodoCount(3);
  await actor.deleteTodoItem(todoItemsFilter[0]);
});
...
Given("User has todos with different statuses", async function () {
  await actor.addTodoItem(todoItemsFilter[0]);
  await actor.addTodoItem(todoItemsFilter[1]);
  await actor.addTodoItem(todoItemsFilter[2]);
